...
public class NeteaseWeibo extends Weibo {

	public NeteaseWeibo(String weiboName, String baseUrl, String factoryName,
	...
	public List<Status> getHomeTimeline(Paging paging) throws HttpException {
		List<Status> statuses =
				mExtractor.extractTimeline(send(HttpMethod.GET, "statuses/home_timeline", paging));
		if(paging != null && paging.getSinceId() != null)
	...
	public void repostStatus(Status status, String repost) throws HttpException {
		send(HttpMethod.POST, "statuses/retweet/" + status.id,
	...
	public void commentStatus(Status status, String comment) throws HttpException {
		send(HttpMethod.POST, "statuses/reply",
				createParams(new BasicNameValuePair("id", status.id),
						new BasicNameValuePair("status", comment)));
	}
	...
	public void uploadPhoto(String status, File photo) throws HttpException {
		Response picres = send(HttpMethod.POST, "statuses/upload",
				createParams(new BasicNameValuePair("status", status)), null,
				"pic", photo);
		String picurl = mExtractor.extractString(picres, "upload_image_url");
		updateStatus(picurl + " " + status);
	}
	...
	protected void setTimelinePaging(ApiRequest request, Paging paging) {
		if(paging.getMaxId() != null)
			request.addQueryParameter("since_id", paging.getMaxId());
		if(paging.getSinceId() != null)
			request.addQueryParameter("max_id", paging.getSinceId());
		if(paging.getPage() != -1)
